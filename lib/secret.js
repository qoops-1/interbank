...
PublicKey.prototype.address = function () {
    const sanitize = true;
    return pubToAddress(this.buffer(), sanitize);
};
...
KeySet.prototype.remove = function (addressString) {
  let nextKeys = []
  let address = Buffer.from(addressString.replace("0x", ""), "hex");
  _.each(this._keys, k => {
      if (!_.isEqual(address, k.address())) {
          nextKeys.push(k)
      }
  })
  this._keys = nextKeys
...
const encrypt = function (plainText, password) {
    let cipher = crypto.createCipher(CIPHER_NAME, password);
    let encrypted = cipher.update(plainText, 'binary', 'binary') + cipher.final('binary');
...
const decrypt = function (cipherText, password) {
    let decipher = crypto.createDecipher(CIPHER_NAME, password);
    let decrypted  = decipher.update(cipherText, 'binary', 'binary') + decipher.final('binary');
 ...
 * @return {Decipher}
 */
const decipher = password => crypto.createDecipher(CIPHER_NAME, password);
