        ...
        this._download(swarmHash, descriptorString => {
            ...
            if (handler) {
              ...
              this._download(descriptor.url, encryptedDocument => {
                ...
                let error = new Error('Can not find a handler for you.')
                callback(error, null)
            }
        });
    })
  ...
  this.download(address, (error, document) => {
      ...
      if (error && document === null) {
        ...
        try {
          ...
          e.message = "KYC form is not a valid JSON"
          action(e, null)
          return
        }
      }
      if (help.currentTokenExpired()) {
          ...
          let newToken = this.web3.eth.contract(TokenContractData.abi).new(2000, {
            ...
            kycForm.tokens.push({address: token.address, mult: token.mult})
            this.uploadAsync(JSON.stringify(kycForm), signTransaction)
            action(null, token)
          })  
      } else {
