...
Client.prototype.upload = function (document, signTransaction, callback) {
    ...
    this._upload(encryptedDocument, documentUrl => {
        ...
        this.swarm.upload(descriptorString, descriptorSwarmHash => {
            this._blockchainWrite(descriptorSwarmHash, checksum, signTransaction);
            callback(checksum, bzzrAddress(descriptorSwarmHash));
        });
    });
};
...
Client.prototype._blockchainWrite = function (descriptorSwarmHash, checksum, signTransaction) {
    let hexDescriptorHash = "0x" + descriptorSwarmHash;
    let hexChecksum = "0x" + checksum.toString("hex");
    this.kycStorage.add(hexDescriptorHash, hexChecksum, signTransaction);
};
